...
class DbQueryBuilder {
    ...
    func GetRoutesByLocation(location:[String: Double]) -> String?{
        ...
        if let radius = whereSeachRadius        () { statement += radius    }
            if let direction = whereDirections  () { statement += direction }
            if let routeType = whereRouteType   () { statement += routeType }
            return statement
        }
    ...
    func getStopsByRoute(route:String?) -> String {
        ...
        if let r = route { statement += " and r.route_id = \'\(r)\' "   }
        if let radius = whereSeachRadius    () { statement += radius    }
        if let direction = whereDirections  () { statement += direction }
        if let routeType = whereRouteType   () { statement += routeType }
    ...
    func whereDirections()  -> String? {
        ...
        if dbParams["N"]! == true   { directions = " 1 "; isFirst = false }
        if dbParams["S"]! == true   {
    ...
    func whereRouteType () -> String? {
        ...
        if dbParams["Bus"]! == true {
            if isFirst == true { routeType = " 3 " ; isFirst = false ; }
        }
        if dbParams["Train"]! == true {
            if isFirst == true { routeType = " 1 " ; isFirst = false ;
            } else { routeType! += " ,1 " ; }
        }
